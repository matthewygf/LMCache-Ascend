...
class CustomAscendCmakeBuildExt(build_ext):
    def build_extension(self, ext):
        ...
        # hccl_pingpong/hccl_onesided need internal hcomm headers. Prefer an
        # explicit checkout path, otherwise use the repo submodule verified above.
        hcomm_src = os.environ.get("HCOMM_SRC_PATH", hcomm_src_path)
        ...
        cmake_cmd += ["  -DUSE_HCCL_ONESIDED=ON"]
        ...
        if self._cann_version_8_5:
            ...
            expected_patterns.append("hccl_onesided_npu_comms*.so")
        else:
            expected_patterns.append("hccl_npu_comms*.so")
